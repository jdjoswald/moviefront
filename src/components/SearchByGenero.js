...
class SearchByGenero extends Component {
    state = {
        all:[],
        id:'1'
    }
    ...
    handleSubmit=(e)=>{
        e.preventDefault()
        const {id}=this.state
        fetch(`http://localhost:8090/api/peliculas/movie/movie/genero/${id}`)
        .then(res=> res.json())
        .then(results=>{
            console.log(results)
